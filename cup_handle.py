...
def _score_pattern(
    ...
    # Prior uptrend strength (20 pts) — O'Neil: this matters most, a cup
    # in a weak/lagging stock is much lower quality regardless of shape.
    score += min(prior_uptrend_pct / 50.0, 1.0) * 20

    # Cup depth quality (15 pts) — ideal zone 15-25%, O'Neil's sweet spot
    ...
    # Cup duration quality (10 pts) — ideal zone 12-26 weeks
    ...
    # Cup volume dry-up at the bottom (10 pts) — confirms selling exhaustion
    if cup_volume_dryup:
        score += 10

    # Handle quality (15 pts split: depth + zone)
    ...
    # Breakout confirmation (10 pts) — genuine volume-backed breakout
    if is_breaking_out:
        score += 10
    elif breakout_volume_ratio >= 1.2:
        score += 4

    # Multi-timeframe alignment (bonus up to 10 pts, daily + monthly trend)
    if daily_confirms:
        score += 5
    if monthly_trend_ok:
        score += 5
