...
class DarvasBox:
    ...
    def quality_fraction(self) -> float:
        """
        quality_score normalised to a 0-1 fraction (quality_score is on
        a fixed internal 0-15 scale — see _calc_quality above). Use this
        in the composite scoring engine so the actual configured
        SCORE_WEIGHTS["box_quality"] is respected, rather than the
        internal 0-15 scale being added directly and silently ignoring
        whatever weight is configured.
        """
        return min(self.quality_score / 15.0, 1.0)
