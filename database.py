...
CREATE TABLE IF NOT EXISTS backtest_runs (
    ...
    pattern_type    TEXT NOT NULL DEFAULT 'darvas_box',  -- 'darvas_box' / 'cup_handle' / 'combined'
...
CREATE TABLE IF NOT EXISTS backtest_symbol_summary (
    ...
    symbol          TEXT NOT NULL,
    pattern_type    TEXT NOT NULL DEFAULT 'darvas_box',
    trades          INTEGER,
    win_rate        REAL,
    profit_factor   REAL,
    cagr_pct        REAL,
    ...
    sharpe          REAL,
    avg_hold        REAL,
    PRIMARY KEY (run_id, symbol, pattern_type)
);
...
CREATE TABLE IF NOT EXISTS backtest_trade_log (
    run_id          TEXT NOT NULL,
    symbol          TEXT NOT NULL,
    pattern_type    TEXT NOT NULL DEFAULT 'darvas_box',  -- 'darvas_box' or 'cup_handle'
    entry_date      TEXT,
    exit_date       TEXT,
    entry_price     REAL,
    exit_price      REAL,
    ...
    rsi_at_entry    REAL,
    adx_at_entry    REAL,
    score_band      TEXT,       -- 'elite' / 'very_strong' / 'strong' / 'watch' / 'below_watch'
    -- Darvas Box specific (NULL for cup_handle trades)
    box_width_pct   REAL,
    box_age_bars    INTEGER,
    box_start_date  TEXT,
    box_end_date    TEXT,
    -- Cup and Handle specific (NULL for darvas_box trades)
    cup_depth_pct       REAL,
    cup_duration_weeks  INTEGER,
    cup_start_date      TEXT,
    cup_bottom_date     TEXT,
    cup_end_date        TEXT,
    handle_depth_pct    REAL,
    handle_duration_weeks INTEGER,
    handle_start_date   TEXT,
    handle_end_date     TEXT,
    prior_uptrend_pct   REAL,
    breakout_volume_ratio REAL,  -- volume surge ratio at the confirmed breakout bar
    pattern_quality     REAL,    -- C&H's own 0-100 quality score (separate scale from composite_score)
    cup_shape_ok         INTEGER,  -- 1/0: rounded U-shape check passed
    cup_volume_dryup     INTEGER,  -- 1/0: volume declined into the cup bottom
    handle_in_upper_zone INTEGER,  -- 1/0: handle formed in the upper half of the cup
    handle_volume_dryup  INTEGER,  -- 1/0: volume declined during the handle
    timeframe            TEXT      -- 'monthly' / 'weekly' / 'daily' (which bars the cup was found on)
);
"""
...
def _migrate_db(con: sqlite3.Connection) -> None:
    ...
    required: dict[str, list[tuple[str, str]]] = {
        ...
        "backtest_trade_log": [
            ...
            ("cup_shape_ok",          "INTEGER"),
            ("cup_volume_dryup",      "INTEGER"),
            ("handle_in_upper_zone",  "INTEGER"),
            ("handle_volume_dryup",   "INTEGER"),
            ("timeframe",             "TEXT"),
        ],
    }
