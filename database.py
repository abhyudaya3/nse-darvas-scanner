...
def save_backtest_trades(run_id: str, trades: list[dict]) -> None:
    """Save per-symbol backtest results for a given run."""
    if not trades:
        return
    with _conn() as con:
        for t in trades:
            ...
            con.execute(
                f"INSERT OR REPLACE INTO backtest_trades ({cols}) VALUES ({ph})", t
            )
...
def get_backtest_trades_df(run_id: str) -> pd.DataFrame:
    with _conn() as con:
        return pd.read_sql(
            "SELECT * FROM backtest_trades WHERE run_id = ? ORDER BY cagr_pct DESC",
            con, params=(run_id,),
        )
