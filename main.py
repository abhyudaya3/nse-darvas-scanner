...
if __name__ == "__main__":
    ...
    if args.backtest_all:
        ...
        init_db()
        log.info("=" * 60)
        log.info("BACKWARD BACKTEST — FULL NSE UNIVERSE")
        log.info("=" * 60)
        ...
        summary = backtest_universe(
            symbols,
            notes=f"limit={args.backtest_limit or 'none'}",
        )
        print("\n=== BACKTEST SUMMARY ===")
        ...
        if summary.get("total_trades", 0) > 0:
            try:
                report_path = generate_backtest_report(summary["run_id"])
                print(f"\n  ✅ Full analysis report saved: {report_path}")
                print(
                    "     Open the 'Verdict' sheet first — it gives a plain-language "
                    "pass/fail against objective criteria, followed by Equity Curve, "
                    "Score Band Analysis (does the score actually predict outcomes), "
                    "Yearly Performance, Per-Symbol Summary, and the full Trade Log."
                )
            except Exception as e:
                log.error("Backtest report generation failed: %s", e)
                print(f"\n  ⚠️  Could not generate analysis report: {e}")
        else:
            print("\n  No trades were generated — analysis report skipped.")
        sys.exit(0)
    ...
    if args.list_backtest_runs:
        ...
        if df.empty:
            ...
            cols = ["run_id", "run_date", "symbols_tested", "total_trades",
                    "win_rate", "profit_factor", "avg_cagr", "notes"]
    ...
    if args.backtest_report:
        ...
        init_db()
        try:
            report_path = generate_backtest_report(args.backtest_report)
            print(f"\n  ✅ Analysis report regenerated: {report_path}")
