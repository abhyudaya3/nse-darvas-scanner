...
if __name__ == "__main__":
    ...
    if args.backtest_all:
        ...
        # Also export to Excel for easy review
        from database import get_backtest_trades_df
        from config import REPORTS_DIR
        trades_df = get_backtest_trades_df(summary["run_id"])
        if not trades_df.empty:
            out_path = REPORTS_DIR / f"backtest_{summary['run_id']}.xlsx"
            trades_df.to_excel(out_path, index=False, sheet_name="Per-Symbol Results")
            print(f"\n  Per-symbol results saved: {out_path}")
        sys.exit(0)
