...
from downloader import load_daily, resample_weekly, resample_monthly, resample_point_in_time
...
def backtest_cup_handle_symbol(
    ...
    # PERFORMANCE FIX 2026-06-25: precompute monthly_full once too. This
    # is now SAFE to use (unlike the reverted attempt on 2026-06-24)
    # because resample_point_in_time() below only ever takes COMPLETE
    # buckets directly from this precomputed series and always
    # recomputes the final, currently-forming bucket fresh from a small
    # daily tail — see resample_point_in_time()'s docstring in
    # downloader.py for the full verification.
    monthly_full = resample_monthly(daily)
    ...
    for week_idx in range(start_week_idx, n_weeks, CH_WALK_STEP_WEEKS):
        ...
        # PERFORMANCE FIX 2026-06-25: replaced full resample_weekly/
        # resample_monthly recomputation (confirmed via profiling to be
        # ~68% of total runtime, and the reason a real NSE-scale run was
        # on track to take 5+ hours — well past the 300-minute workflow
        # timeout) with resample_point_in_time(), which slices completed
        # buckets from the precomputed full-history series and only
        # recomputes the small, currently-forming final bucket. Verified
        # exhaustively to produce IDENTICAL results to full recomputation
        # (see downloader.py's resample_point_in_time docstring) — this
        # is a speed fix, not a correctness tradeoff.
        weekly_slice = resample_point_in_time(daily, weekly_full, as_of_date, "W-FRI")
        monthly_slice = resample_point_in_time(daily, monthly_full, as_of_date, "MS")

        try:
            pattern = detect_cup_and_handle(symbol, daily_slice, weekly_slice, monthly_slice, benchmark=benchmark)
        except Exception as e:
        ...
        trade_log.append({
            ...
            # ADDED 2026-06-25: which bar timeframe (monthly/weekly/daily)
            # this specific trade's cup was found on. Needed now that
            # detect_cup_and_handle() searches all three independently —
            # without this, the backtest couldn't tell whether monthly-
            # scale, weekly-scale, or daily-scale cups perform differently.
