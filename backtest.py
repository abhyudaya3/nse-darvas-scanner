...
def backtest_cup_handle_symbol(
    ...
    # weekly_full IS still used below, but ONLY to derive the walk-forward
    # loop's as_of_date values (weekly_full.index[week_idx]) — never to
    # slice actual OHLCV data for a given point in time. See the
    # CORRECTNESS FIX comment inside the loop for why both weekly_slice
    # and monthly_slice must be recomputed fresh from daily_slice on
    # every step, not sliced from any precomputed full-history series.
    ...
    for week_idx in range(start_week_idx, n_weeks, CH_WALK_STEP_WEEKS):
        ...
        # CORRECTNESS FIX 2026-06-24: weekly_slice was previously computed
        # as `weekly_full.loc[:as_of_date]` — slicing a weekly series that
        # was resampled ONCE from the complete daily history. This is a
        # genuine, pre-existing lookahead bug, confirmed to affect EVERY
        # SINGLE walk-forward step (not an edge case): resample_weekly()
        # uses label="left", closed="left" on a W-FRI bucket, which means
        # the bar LABELED with a given Friday's date actually aggregates
        # trading days running FORWARD from that label through the
        # following week — so slicing the full series "up to as_of_date"
        # still included future days' High/Low/Close/Volume inside that
        # final bucket. Verified exhaustively: 186/186 walk steps tested
        # on a real multi-year history showed a mismatch between this
        # slicing approach and correctly recomputing from a truncated
        # daily slice. This means every prior Cup & Handle backtest run
        # had access to future price data when evaluating the final
        # (most recent) week of whatever point-in-time slice it was
        # looking at — exactly the kind of leak that inflates backtest
        # results dishonestly. Always recompute from daily_slice instead.
        weekly_slice = resample_weekly(daily_slice)
