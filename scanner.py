...
from config import (
    ...
    EMA_TREND, ENTRY_ZONE_PCT, MIN_SIGNAL_SCORE, RISK_PER_TRADE_PCT,
    RS_MIN_PREFERRED, RSI_MAX, RSI_MIN, RSI_PERIOD, SCORE_THRESHOLDS,
    SCORE_WEIGHTS, VOLUME_MA_FAST, VOLUME_MA_PERIOD, VOLUME_RATIO_MIN,
    RS_WEIGHTS,
)
...
class Signal:
    ...
    box_start_date:  Optional[date] = None   # when the box's left-side high formed
    box_end_date:    Optional[date] = None   # last bar of the consolidation
...
def scan_symbol(
    ...
    *rs_override* — if provided, uses this RS Rating (typically computed
    via the true cross-sectional percentile rank across the whole
    universe — see indicators.cross_sectional_rs_rating) instead of the
    single-stock-vs-benchmark sigmoid estimate. The orchestrator
    (main.py) computes this in a first pass over all symbols before
    calling scan_symbol() for the full filter chain.
    """
    ...
    # Live output gate — tuned from backtest evidence, see config.py for
    # the full cutoff-vs-win-rate table this default (85) was chosen from.
    if score < MIN_SIGNAL_SCORE:
        log.debug("%s: skip — score %.1f below live output threshold (%.0f)",
                  symbol, score, MIN_SIGNAL_SCORE)
        return None
...
def _compute_score(
    ...
    # 5. Box quality (configured weight, default 13 pts) — FIXED 2026-06-20.
    # Previously this added box.quality_score (a fixed internal 0-15
    # scale) directly, completely ignoring whatever SCORE_WEIGHTS
    # ["box_quality"] was configured to — so changing that weight had
    # NO effect at all. Now uses box.quality_fraction (0-1) × the actual
    # configured weight, consistent with every other scoring factor below.
    score += w["box_quality"] * box.quality_fraction
    ...
    # 7. RSI reversal quality (20 pts) — REBALANCED 2026-06-20.
    # Real backtest evidence (5,175 trades) showed win rate climbing
    # MONOTONICALLY with RSI across the entire allowed entry range:
    #   RSI 25-30: 39.6% win rate    RSI 45-50: 64.8% win rate
    #   RSI 30-35: 39.9% win rate    RSI 50-55: 69.5% win rate
    #   RSI 35-40: 47.4% win rate    RSI 55+:   73.3% win rate
    #   RSI 40-45: 52.0% win rate
    # The PREVIOUS scoring curve had this backwards — it gave FULL credit
    # to RSI 35-48 (mid-range, ~47-58% actual win rate) and only PARTIAL
    # credit to RSI 50-55 (the actual best-performing entries at 69.5%).
    # This curve now scores linearly toward the top of the allowed RSI
    # band, since that's where the real edge concentrates.
    if RSI_MIN <= rsi <= RSI_MAX:
        # Linear scale: RSI_MIN -> 0.3 floor, RSI_MAX -> 1.0 ceiling.
        # A floor of 0.3 (not 0.0) still gives oversold entries SOME
        # credit, since the strategy's stop-loss/target structure can
        # still produce a positive expectancy even at lower win rates —
        # this isn't a hard filter, just a quality weighting.
        span = RSI_MAX - RSI_MIN
        rsi_pts = 0.3 + 0.7 * ((rsi - RSI_MIN) / span) if span > 0 else 0.5
    else:
        rsi_pts = 0.0
