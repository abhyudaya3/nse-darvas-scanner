...
class CupHandleSignal:
    ...
    # ADDED 2026-06-25: which bar timeframe this pattern was found on
    # (monthly/weekly/daily). See cup_handle.py's detect_cup_and_handle()
    # docstring for the full multi-timeframe redesign this supports.
    timeframe:          str
    ...
    nested_pattern_count: int = 0     # other valid candidates found on different timeframes
    signal_id:          str = ""
...
def scan_cup_handle(
    ...
    *rs_override* — if provided, uses this RS Rating instead of the
    ...
    pattern = detect_cup_and_handle(symbol, daily, weekly, monthly, benchmark=benchmark)
    if pattern is None:
        return None
    ...
    sig = CupHandleSignal(
        ...
        nested_pattern_count = len(pattern.nested_patterns),
    )
